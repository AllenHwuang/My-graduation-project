<template>
	<div style="width: 650px; margin-top: 60px;">
...
export default {
  ...
  data () {
	...
	mounted () {
      ...
      this.initChart2()
			this.myChart2 = echarts.init(document.getElementById('myChart2'), 'light', {renderer: 'svg'}),
	...
	methods: {
		initChart2 () 
		{
			...
			var sValue = this.sizeValue;
			var sySize = this.symbolSize;
			console.log("testv:" + sValue);
			this.option.grid[0].right = sValue;
			this.option.grid[0].bottom = sValue;
			this.option.grid[1].left = sValue;
			this.option.grid[1].bottom = sValue;
			this.option.grid[2].right = sValue;
			this.option.grid[2].top = sValue;
			this.option.grid[3].left = sValue;
			this.option.grid[3].top = sValue;
			this.option.series[0].symbolSize = sySize
			this.option.series[1].symbolSize = sySize
			this.option.series[2].symbolSize = sySize
			this.option.series[3].symbolSize = sySize
			this.option.dataset.source = this.data
			console.log("grid--right: " + this.option.grid[0].right);
			console.log("serier--symbolSize: " + this.option.series[0].symbolSize);
			//图标根据窗口大小自动缩放
